...
import { FormDataContract } from "../../pages/Contracts/pages/CreatePriceFixationContract/types";
import { parseQuantityToNumber } from "../quantityFormat";

export const FormDataToIPriceFixationContractDataDTO = (
  ...
  return {
    ...
    // Preço de referência opcional (não é uma fixação de mercado).
    price: data.price?.trim() ? parseFloat(data.price.replace(",", ".")) : undefined,

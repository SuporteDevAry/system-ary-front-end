...
import { IContatosProvider, ICreateContatosData, IUpdateContatosData } from "./types";
import { Api } from "../../services/api"; 

interface IContatosContext {
  listContatos: () => Promise<any>;
  createContato: (contatoData: ICreateContatosData) => Promise<any>;
  updateContato: (contatoCli_codigo: string, contatoSequencia: string,  updateContatoData: any) => void;
...
export const ContatosProvider = ({ children }: IContatosProvider) => {
  ...
  async function updateContato(contatoCli_codigo: string, contatoSequencia: string, updateContatoData: IUpdateContatosData) {
    try {
      const response = await Api.patch(`/contato/${contatoCli_codigo}/${contatoSequencia}`, updateContatoData);
      return response;
    } catch (error) {
  ...
  async function deleteContato(contatoCli_codigo: string, contatoSequencia: string) {
    try {
      const response = await Api.delete(`/contato/${contatoCli_codigo}/${contatoSequencia}`);
  ...
  return (
    <newContext.Provider
      value={{ listContatos, createContato, updateContato, deleteContato }}
    >
      {children}
    </newContext.Provider>
  );

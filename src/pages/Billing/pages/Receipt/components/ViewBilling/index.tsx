...
const sanitizePatchPayload = <T extends Record<string, any>>(
  payload: T,
): Partial<T> =>
  Object.fromEntries(
...
export function ViewBilling(): JSX.Element {
    ...
    try {
      ...
      const filteredBillings = response.data
        ...
        .map((bill: any) => ({
          ...
          //     "Real"
          // ),
          total_service_value: Number(bill.total_service_value).toFixed(2),
  ...
  const handleConfirmBilling = async () => {
    ...
    try {
      if (billingToEdit?.id) {
        ...
        if (res.status === 200) {
          ...
          const totReceivedContract = resTotalContract.data.reduce(
            (acc: number, item: { liquid_value: any }) =>
              acc + parseFloat((item.liquid_value || "0").replace(",", ".")),
            0,
          );

          const totalReceived = totReceivedContract.toFixed(2);
          const newContractForToUpdate = sanitizePatchPayload<
            Partial<IContractData>
          >({
          ...
          if (
            ...
            const updatedStatus = {
              ...
              history: [...resContract.data.status.history, newStatusEntry],
            };
        ...
        if (res.status === 201) {
          const totalReceived =
            Number(dataClient?.total_received) + Number(formData.liquid_value);
          const updatedContract = sanitizePatchPayload<Partial<IContractData>>({
            day_exchange_rate: undefined,
            total_received: totalReceived,
            status_received: formData.liquid_contract,
          });
  ...
  const handleDeleteBilling = async () => {
    try {
      ...
      const resTotalContract =
        await billingContext.getBillingByNumberContract(req_number_contract);

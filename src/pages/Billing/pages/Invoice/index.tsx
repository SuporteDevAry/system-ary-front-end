...
export function Invoice() {
  ...
  const gerarXML = () => {
    ...
    rpsList.forEach((rps) => {
      ...
      // Alíquota em formato decimal (ex: 2% = 0.0200)
      const aliquota = Number(rps.aliquota || 0.02); // Padrão 2% para SP
      const aliquotaFormatada =
        aliquota > 1 ? (aliquota / 100).toFixed(4) : aliquota.toFixed(4);

      console.log(
        `RPS ${rps.rps_number} - Alíquota DB: ${rps.aliquota} → Formatada: ${aliquotaFormatada}`
      );
      ...
      xml += `<DataEmissao>${dayjs(rps.rps_emission_date).format(
        "YYYY-MM-DD"
      )}</DataEmissao>`;
      ...
      xml += `<ValorPIS>${Number(rps.valor_pis || 0).toFixed(2)}</ValorPIS>`;
      xml += `<ValorCOFINS>${Number(rps.valor_cofins || 0).toFixed(
        2
      )}</ValorCOFINS>`;
      xml += `<ValorINSS>${Number(rps.valor_inss || 0).toFixed(2)}</ValorINSS>`;
      xml += `<ValorIR>${Number(rps.valor_ir || 0).toFixed(2)}</ValorIR>`;
      xml += `<ValorCSLL>${Number(rps.valor_csll || 0).toFixed(2)}</ValorCSLL>`;
      ...
      if (rps.email && rps.email.trim()) {
        xml += `<EmailTomador>${escapeXml(rps.email.trim())}</EmailTomador>`;
      }

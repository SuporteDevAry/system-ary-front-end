...
export function Invoice() {
  ...
  const handleIssueFull = async () => {
    ...
    try {
      setIsLoadingRPS(true);
      const resp = await fetch("http://localhost:3030/api/nfse/enviar-lote", {
        method: "POST",
      ...
      if (!resp.ok) {
        toast.error("Erro no envio: " + (json.error || JSON.stringify(json)));
      } else {
  ...
  const nameColumnsFromRPS = useMemo(
    () => [
      { field: "rps_number", header: "RPS" },
      {
        ...
        header: "Dt.RPS",
        headerTooltip: "Data de Emissao do RPS",
      },

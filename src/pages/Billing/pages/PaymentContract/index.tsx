...
const buildFilterSummaryLines = (filters: SelectState) => {
    ...
    const chargeStart =
        formatIsoDateToBR(filters.charge_date_start) || "99/99/9999";
    const chargeEnd =
        formatIsoDateToBR(filters.charge_date_end) || "99/99/9999";
...
export function PaymentContract() {
    ...
    const processedContracts = useMemo(() => {
        ...
        const emissionStart = selectData.date_start
            ? parseBrazilianDate(
                  formatIsoDateToBR(selectData.date_start) ||
        ...
        processedData = processedData.filter((contract) => {
            const emissionDate = parseBrazilianDate(
            ...
            const matchEmissionStart = emissionStart
                ? emissionDate >= emissionStart
                : true;
            const matchEmissionEnd = emissionEnd
                ? emissionDate <= emissionEnd
                : true;
            const matchChargeStart = chargeStart
                ? chargeDate >= chargeStart
                : true;
    ...
    const handleExportExcelStyled = () => {
        try {
            ...
            for (let col = 0; col < columnCount; col += 1) {
                ...
                applyStyle(
                    XLSX.utils.encode_cell({ r: headerRowIndex, c: col }),
                    headerStyle,
                );
            }

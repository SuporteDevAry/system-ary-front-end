...
export function Invoicing() {
  ...
  const fetchTables = useCallback(async () => {
    try {
      ...
      const defaultMesa =
        tables.find(
          (mesa: Mesa) => normalizeText(mesa.name) === normalizeText("Grãos"),
        ) ?? tables[0];
    ...
    (filters: SelectState, showToast = true) => {
      try {
        ...
        const filtered = allContracts.filter((contract) => {
          const contractDate = contract.contract_emission_date?.trim();
        ...
        const currentMesa =
          tables.find((mesa) =>
            sameProductTypes(mesa.product_types, selectedProductTypes),
          ) ??
          tables.find((mesa) =>
            sameProductTypes(
              mesa.product_types,
  ...
  const isInitialFilter = useMemo(() => {
    return (
      ...
      sameProductTypes(selectData.product_types, defaultFilters.product_types)
    );
  ...
  const pivotData = useMemo(() => {
    ...
    const productOrder =
      selectedProducts.length > 0
        ...
        : Array.from(
            new Set(
              filteredData
                .map((contract) => normalizeText(contract.product))
                .filter(Boolean),
            ),
          ).sort();
    ...
    productOrder.forEach((product) => {
      const brokers = Array.from(brokerOrderByProduct.get(product) ?? []).sort(
  ...
  const handleExportExcel = () => {
    try {
      ...
      for (let col = 0; col < columnCount; col += 1) {
        ...
        applyStyle(XLSX.utils.encode_cell({ r: 2, c: col }), headerStyle);
      }

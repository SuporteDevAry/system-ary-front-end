...
export function GrainsVol() {
  ...
  const fetchSelectData = useCallback(
    (filters: SelectState, showToast = true) => {
      try {
        ...
        const nameProductTerm = normalizeText(filters.name_product as string);
  ...
  const reportRows = useMemo(() => {
    ...
    sortedRows.forEach((row) => {
      ...
      grandTotals.commission_value += parseLocaleNumber(row.commission_value);
      siglaTotals.total_contract_real += parseLocaleNumber(
        row.total_contract_real,
      );
      siglaTotals.commission_value += parseLocaleNumber(row.commission_value);
      siglaTotals.quantity += quantityValue;
      monthTotals.total_contract_real += parseLocaleNumber(
        row.total_contract_real,
      );
      monthTotals.commission_value += parseLocaleNumber(row.commission_value);
    ...
    sortedRows.forEach((row, index) => {
      ...
      const isLastRowInMonth = sigla !== nextSigla || monthKey !== nextMonthKey;
  ...
  const handleExportExcel = () => {
    try {
      ...
      for (let col = 0; col < columnCount; col += 1) {
        ...
        applyStyle(XLSX.utils.encode_cell({ r: 1, c: col }), titleStyle);
      ...
      for (let row = 4; row < exportRows.length; row += 1) {
        ...
        for (let col = 0; col < columnCount; col += 1) {
          const baseStyle =
            col === 0 || nameColumns[col].field === "contract_emission_date"
              ? textStyle
              : numberStyle;
          const style = isTotalRow
            ...
            : baseStyle;
          applyStyle(XLSX.utils.encode_cell({ r: row, c: col }), style);
        }
      }

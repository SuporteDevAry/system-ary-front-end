...
export function ViewContract(): JSX.Element {
  ...
  useEffect(() => {
    ...
    console.log("Contrato para Visualizar:", contractForView);
  ...
  const formattedValue = formatValue(dataClient?.quantity ?? 0);
  const formattedValueFinal = formatValue(dataClient?.final_quantity ?? 0);
  ...
  const contractFields = [
    ...
    {
      label: "Quantidade",
      value: `${replaceLastDotWithComma(formattedValue)} ${unityMeasure}`,
    },
    {
      label: "Quantidade Final",
      value: `${replaceLastDotWithComma(formattedValueFinal)} ${unityMeasure}`,
    },
